...
Next, we estimate the effect of the tax on sugar purchases (from all food sources) using Equation \ref{spec_did}. To instill some confidence that our comparison group is a credible counterfactual, we plot the time series of average monthly sugar purchases per household in Figure \ref{cook_panelist_nutr}. One can see that % talk about pretrends
...
% Our estimates for Equation \ref{spec_did} using both total sugar and sugar from soda as outcome variables can be seen in Table \ref{asdf}. We find that during the taxed period, consumers purchased TODO\% less sugar overall when compared to purchases by consumers living in untaxed counties. Interestingly, we find that TODO\% of the reduction in total sugar is explained by the reduction of sugar from regular carbonated beverages.
